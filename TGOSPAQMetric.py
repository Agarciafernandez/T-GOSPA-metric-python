#This code is a python implementation of the trajectory GOSPA (T-GOSPA) quasi-metric between sets of trajectories.
...
# The code is based on a translation of the MATLAB implementation of the LP T-GOSPA metric 
# LPTrajMetric_cluster.m available at 
# https://github.com/Agarciafernandez/MTT/tree/master/Trajectory%20metric
# The authors of this Python implementation are  Á. F. García-Fernández and Jinhao Gu
...
def computeLocCostPerTime(x,y,c,p,cost_missed_single,cost_false_single):
    if np.all(np.invert(np.isnan(x))) and np.all(np.invert(np.isnan(y))):
        #neither x nor y has hole
        d=min(np.linalg.norm(x-y,p)**p,c**p)
    
    elif np.any(np.logical_and(np.isnan(x),np.invert(np.isnan(y)))): 
        #X has a hole. Y does not have a hole
        d=cost_false_single
        
    elif np.any(np.logical_and(np.isnan(y),np.invert(np.isnan(x)))): 
...
def locCostComp_V2(X,Y,c,p,cost_missed_single,cost_false_single):
    '''
                ...
                # if X_xind exists at time t
                    for yind in range(ny+1):
                            if yind <=ny-1 and t+1>=Y.tVec[yind] and t+1<=Y.iVec[yind]+Y.tVec[yind]-1:
                                ...
                                locCostMat[xind,yind,t]=computeLocCostPerTime(X.xState[:,t,xind],Y.xState[:,t,yind],c,p,cost_missed_single,cost_false_single)
                                
                            else:
                                locCostMat[xind,yind,t]=cost_missed_single
                else:
                    #X_xind does not exist or yind is dummy
...
def computeLocFalseMissedSwitchCosts(wMat,LocCostMat,X,Y,c,p,gamma,cost_missed_single,cost_false_single):
    '''
    ...
    miss_cost=cost_missed_single*np.sum(np.sum(miss_mask*wMat,axis=1),axis=0)+cost_missed_single*np.sum(np.sum(fa_miss_mask*wMat,axis=1),axis=0)
    fa_cost=cost_false_single*np.sum(np.sum(fa_mask*wMat,axis=1),axis=0)+cost_false_single*np.sum(np.sum(fa_miss_mask*wMat,axis=1),axis=0)
    
    return loc_cost,miss_cost,fa_cost,switch_cost


                     
def LP_metric_Cluster(X,Y,DAB,nx,ny,nxny,nxny2,T,c,p,gamma,cost_missed_single,cost_false_single):
    ''' 
    ...
    f=np.zeros((nParam,1))
    f[WtPos]=np.reshape(DAB,(WtLen,1),order='F')# for vec(W(1)) to vec(W(T))
    f[etPos]=0.5*gamma**p * np.ones((T-1,1))
    ...
    loc_cost,miss_cost,fa_cost,switch_cost=computeLocFalseMissedSwitchCosts(wMat,DAB,X,Y,c,p,gamma,cost_missed_single,cost_false_single)
    
    return dxy,loc_cost,miss_cost,fa_cost,switch_cost



def LPTrajQMetric_Cluster(X,Y,c,p,gamma,rho):
    '''
    This function computes the trajectory generalised optimal subpattern assignment metric (T-GOSPA) between sets of trajectories.
    In particular, it uses the linear programming (LP) implementation in
    in  Á. F. García-Fernández, A. S. Rahmathullah and L. Svensson,
    ...
    for i in range(len(Clusters)):
        ...
        if (len(Cluster_i)==1):
            ...
            if len(i_x)==0:
                ...
                dxy_i=cost_false_single*np.sum(np.squeeze(isrealY_i))
                fa_cost[t_axis]=fa_cost[t_axis]+cost_false_single*isrealY_i.T
            else:
                ...
                dxy_i=cost_missed_single*np.sum(np.squeeze(isrealX_i))
                miss_cost[t_axis]=miss_cost[t_axis]+cost_missed_single*isrealX_i.T
            ...
            dxy_i=np.sum(DAB[list_x,list_y,:])
            t_axis=np.arange(X.tVec[list_x],X.tVec[list_x]+X.iVec[list_x])-1
            ...
            index2=np.squeeze(np.logical_and(DAB_i==cost_false_single,np.logical_or(isnan_X,np.transpose(no_exist_X))))
            fa_cost[index2]=fa_cost[index2]+cost_false_single

            
            #Missed targets (when Y does not exist)
            index3=np.squeeze(np.logical_and(DAB_i==cost_missed_single,np.logical_or(isnan_Y,np.transpose(no_exist_Y))))
            ...
            for j in range(len(X_i.tVec)):
                ...
                miss_cost_i[t_axis]=miss_cost_i[t_axis]+cost_missed_single*np.squeeze(isrealX_i[0:1,t_axis,j])
            ...
            for j in range(len(Y_i.tVec)):
                ...
                fa_cost_i[t_axis]=fa_cost_i[t_axis]+cost_false_single*np.squeeze(isrealY_i[0:1,t_axis,j])
